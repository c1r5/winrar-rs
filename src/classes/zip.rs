...
use crate::interfaces::archive::{ArchiveReader, ArchiveFileInfo, ArchiveInfo};
use zip::ZipArchive;
...
impl ArchiveReader for ArchiveFileZip {
    fn info(&mut self) -> std::io::Result<ArchiveInfo> {
        ...
        Ok(ArchiveInfo { length, comment, has_password: has_password.is_err() })
    }

    fn enumerate_files(&mut self) -> std::io::Result<Vec<ArchiveFileInfo>> {
